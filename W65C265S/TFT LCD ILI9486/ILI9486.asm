; ************* TO DO ********************************************************
; -test without ili_VIA2_PORTB_SHADOW, reading port directly
; -
; ****************************************************************************
...
; Connecting to second VIA (VIA2) using port A for data and port B for control.
...
; VIA is initialized in the main assembly file (1602_LCD_PS2KBD.asm)
;   equs copied below for reference
; 	 VIA2 Address - %11010000:00000000:00000000 - $D0:0000
;    VIA2_ADDR:  	equ $D00000
;    VIA2_PORTB: 	equ $D00000
;    VIA2_PORTA: 	equ $D00001
;    VIA2_DDRB:  	equ $D00002
;    VIA2_DDRA:  	equ $D00003
;
...
ILI_Init:
    ...
    stz ili_VIA2_PORTB_SHADOW           ; 16-bit (only need LSB though)
    stz ili_rect_height
    stz ili_rect_width
    stz ili_color
...
ILI_Write_Command:
    ...
    sta >VIA2_PORTA         ; put the data on the data bus to the ILI
    jsr ILI_Pulse_WR
...
ILI_Write_Data_Word:
    ...
    Inner_Loop:             ; X loop (width)
	    ...
	    sep #$20		    ; 8-bit A    
        LONGA OFF
        sta >VIA2_PORTA     ; put data on the data base to the ILI
        jsr ILI_Pulse_WR
        rep #$20		    ; 16-bit A
        LONGA ON
