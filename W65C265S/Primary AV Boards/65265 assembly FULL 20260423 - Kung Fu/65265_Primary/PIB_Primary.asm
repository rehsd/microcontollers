...
;       Using $F0:0700 to $F0:07FF for command/parameter passing from slave to slave (256 bytes)
;           $F0:0700 = slave busy
; ***************************************************************************
...
Init_PIB_master:
    ...
    lda #0
    sta $F00700     ; slave status      bit0 = slave busy flag (0=ready, 1=busy)
    sta $F00701     ; slave status
...
pib_busy_wait:
    ; caller guarantees 8-bit A
    .setting "RegA16", false
    pib_wait_not_busy:
        lda $F00700
        and #%00000001    ; check slave busy flag
        bne pib_wait_not_busy
    rts
...
pib_set_busy:
    ...
    @RetryWrite:    ; in case the write fails (dp sram conflict?)
        ...
        ; value was 0 → print it before retrying
...
pib_draw_line:
    ;Before calling, write to dpsram ($F00000): start_x_lo, start_x_hi, start_y_lo, start_y_hi, width_lo, width_hi, height_lo, height_hi, color_lo, color_hi, ----
    php
    sep #$30		; 8-bit A/X/Y
    .setting "RegA16", false
    .setting "RegXY16", false
...
pib_draw_circle:
    ;Before calling, write to dpsram ($F00000): start_x_lo, start_x_hi, start_y_lo, start_y_hi, radius_lo, radius_hi, ----, ----, color_lo, color_hi, filled
    php
    sep #$30		; 8-bit A/X/Y
    .setting "RegA16", false
    .setting "RegXY16", false
...
pib_draw_pixel:
    ;Before calling, write to dpsram ($F00000): start_x_lo, start_x_hi, start_y_lo, start_y_hi, ----, ----, ----, ----, color_lo, color_hi, ----
    php
    sep #$30		; 8-bit A/X/Y
    .setting "RegA16", false
    .setting "RegXY16", false
...
pib_set_char_xy:
    ;Before calling, write to dpsram ($F00000): char_vp_x_lo, char_vp_x_hi, char_vp_y_lo, char_vp_y_hi, ----, ----, ----, ----, ----, ----, ----
    php
    sep #$30		; 8-bit A/X/Y
    .setting "RegA16", false
    .setting "RegXY16", false
...
pib_set_char_color:
    ;Before calling, write to dpsram ($F00000): ----, ----, ----, ----, color_lo, color_hi, ----, ----, ----, ----, ----
    php
    sep #$30		; 8-bit A/X/Y
    .setting "RegA16", false
    .setting "RegXY16", false
...
pib_draw_sprite_32:
    ;Before calling, write to dpsram ($F00000): start_x_lo, start_x_hi, start_y_lo, start_y_hi, sprite_id_lo, sprite_id_hi, ----, ----, ----, ----, ----
    php
    sep #$30		; 8-bit A/X/Y
    .setting "RegA16", false
    .setting "RegXY16", false
...
pib_tile_restore_32x44:
    ...
    ldx #PIB_CMD_HI         ; PIRS2
    lda #>PIB_CMD_TILE_RESTORE_32x44
    jsr send_pib
...
pib_tile_backup_32x44b:
    ...
    ldx #PIB_CMD_HI         ; PIRS2
    lda #>PIB_CMD_TILE_BACKUP_32x44B
    jsr send_pib
...
pib_tile_restore_32x44b:
    ...
    ldx #PIB_CMD_HI         ; PIRS2
    lda #>PIB_CMD_TILE_RESTORE_32x44B
    jsr send_pib
