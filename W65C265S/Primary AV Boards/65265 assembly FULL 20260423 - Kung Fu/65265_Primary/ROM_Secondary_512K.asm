...
SYSCALL_PARAMS   			.equ 	$0F00           ; 256 bytes reserved for passing parameters to OS syscalls (and scratch space for shell)
...
ShellLoader:
    ...
    lda #$0000
    sta hex_byte_count  ; Reset count
    sta hex_record_type ; Reset type
    sta hex_temp_byte   ; Reset scratchpad
    sta hex_entry_ptr
    sta hex_entry_ptr+1
    ...
    call_shell:
        ...
        // cop COP_CMD_DEBUG_MARK
        
        // lda #$4000                  ; set C direct page
        // tcd                               

        // cop COP_CMD_DEBUG_MARK2
...
.org $004000    ; really $003000 + $40:0000 = $40:3000 in app address space (secondary flash)
ShellHex:
    shell_hex_start:
