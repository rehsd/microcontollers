...
void start_shooter2()
{
    ...
    int old_x = nes_x;
    int old_y = nes_y;
    unsigned char old_pad = 0;
    char songNum = 2;
    char num_str[4]; // Buffer for 3 digits + null terminator

    os_play_music(songNum);

    // Get the initial backup of the empty screen
    os_tile_backup_8(nes_x, nes_y);
    os_draw_sprite8(nes_x, nes_y, SPRITE_8_SHOOTER_CROSSHAIR);
    ...
    while(1) {
        ...
        // 1. INPUT & MOVEMENT
        int moved = 0;
        if (pad & NES_UP)    { nes_y--; moved = 1; }
        if (pad & NES_DOWN)  { nes_y++; moved = 1; }
        if (pad & NES_LEFT)  { nes_x--; moved = 1; }
        if (pad & NES_RIGHT) { nes_x++; moved = 1; }
        ...
        // 2. THE GRAPHICS ATOMIC BLOCK
        // Only run if we moved OR if a button is currently held (drawing)
        if (moved || (pad & (NES_A | NES_B))) {
            
            // A. Remove the crosshair from where it was
            os_tile_restore_8(old_x, old_y);

            // B. Draw the permanent shape at the CURRENT position
            if ((pad & NES_A) && !(pad & NES_B)) {
                os_draw_circle(nes_x, nes_y, 3, RGB332_RED, FILLED);
            }
            else if (!(pad & NES_A) && (pad & NES_B)) {
            ...
            // C. Backup the background at the NEW position (includes the shape)
            os_tile_backup_8(nes_x, nes_y);

            // D. Overlay the crosshair
            os_draw_sprite8(nes_x, nes_y, SPRITE_8_SHOOTER_CROSSHAIR);

            // E. Lock the coordinates for the next 'Restore' call
            old_x = nes_x;
            old_y = nes_y;
        }

        // 3. THE AUDIO ATOMIC BLOCK (Rising Edge Only)
        if ((pad & NES_A) && !(old_pad & NES_A)) {
            os_play_sfx(2);
        }
        if ((pad & NES_B) && !(old_pad & NES_B)) {
            os_play_sfx(4);
        }
        ...
        if (pad & NES_START) {
            ...
            os_play_music(songNum);
            os_puts_serial("Playing song number: ");
            int_to_ascii(songNum, num_str);
            os_puts_serial(num_str);
            os_newline_serial();
        }
...
// {
//     os_draw_rectangle(0, 0, 319, 239, RGB332_SKY_BLUE, FILLED);
//     os_puts("A little Kung Fu...");

//         os_play_music(17);

//         nes_x = 160;
//         nes_y = 120;
//         int old_draw_x = nes_x;
//         int old_draw_y = nes_y;
//         unsigned char old_pad = 0;

//         char lastWalkingFrame = 0;
//         int facingRight = 0;
//         int lastSpriteDrawn = -1; 

//         os_tile_backup_32x44(nes_x, nes_y);
...
//             // 2. SPRITE SELECTION & OFFSET LOGIC
//             if (pad & NES_DOWN) {
//                 if (pad & NES_A) {
//                     spriteToShow = 7;
//                     offsetX = -4; 
...
//             }
//             else if (moved) {
//                 lastWalkingFrame = !lastWalkingFrame;
//                 spriteToShow = lastWalkingFrame;
//             }
//             else {
//                 spriteToShow = 0;
//             }
...
//             // 3. BOUNDS
//             if (nes_x < 0) nes_x = 0;
//             if (nes_x > 287) nes_x = 287;

//             int current_draw_x = nes_x + offsetX;
//             int current_draw_y = nes_y + offsetY;

//             // 4. THE GRAPHICS BLOCK
//             if (current_draw_x != old_draw_x || current_draw_y != old_draw_y || spriteToShow != lastSpriteDrawn) {
//                 os_tile_restore_32x44(old_draw_x, old_draw_y);
//                 os_tile_backup_32x44(current_draw_x, current_draw_y);
//                 os_draw_sprite32x44(current_draw_x, current_draw_y, spriteToShow);

//                 old_draw_x = current_draw_x;
//                 old_draw_y = current_draw_y;
//                 lastSpriteDrawn = spriteToShow;
...
//             // 5. THE AUDIO BLOCK (Rising Edge Trigger - Play as he strikes)
//             if ((pad & NES_A) && !(old_pad & NES_A)) {
//                 os_play_sfx(5);
//             }
//             if ((pad & NES_B) && !(old_pad & NES_B)) {
...
char music_playing_kungfu = 0;
unsigned short time_remaining_kungfu = 2000;
unsigned short score_kungfu = 0;

static void draw_kungfu_background()
{
    os_draw_rectangle(0, 0, 319, 239, RGB332_SKY_BLUE, FILLED);

    os_set_char_xy(10, 2);
    os_set_char_color(RGB332_GREEN);
    os_puts("1-000000");

    os_set_char_xy(120, 2);
    os_set_char_color(RGB332_RED);
    os_puts("TOP-008000");

    os_set_char_xy(240, 2);
    os_set_char_color(RGB332_GREEN);
    os_puts("2-000000");
    ...
    os_set_char_xy(10, 26);
    os_set_char_color(RGB332_ORANGE);
    os_puts(" ENEMY");

    os_draw_rectangle(48, 14, 40, 7, RGB332_RED, FILLED);
    os_draw_rectangle(48, 26, 40, 7, RGB332_ORANGE, FILLED);

    os_draw_rectangle(138, 14, 7, 7, RGB332_BLUE, FILLED);
    os_draw_rectangle(151, 14, 7, 7, RGB332_BLUE, FILLED);
    os_draw_rectangle(164, 14, 7, 7, RGB332_BLUE, FILLED);
    os_draw_rectangle(177, 14, 7, 7, RGB332_BLUE, FILLED);
    os_draw_rectangle(190, 14, 7, 7, RGB332_BLUE, FILLED);
    os_set_char_color(RGB332_ORANGE);
    os_set_char_xy(146, 14);
    os_print_char('-');
    os_set_char_xy(159, 14);
    os_print_char('-');
    os_set_char_xy(172, 14);
    os_print_char('-');
    os_set_char_xy(185, 14);
    os_print_char('-');

    os_set_char_xy(250, 14);
    os_set_char_color(RGB332_RED);
    os_puts("TIME");

    // os_set_char_xy(250, 20);
    // os_set_char_color(RGB332_WHITE);
    // os_puts("2000");

    os_draw_rectangle(0, 40, 320, 8, RGB332_GREEN, FILLED);
    for(int t = 4; t < 320; t+=10) {
        os_draw_diamond(t, 44, 4, RGB332_DARK_GREEN, UNFILLED);
    }

    os_draw_rectangle(0, 48, 320, 1, RGB332_BLACK, FILLED);
    os_draw_rectangle(0, 49, 320, 3, RGB332_RED, FILLED);
    os_draw_rectangle(0, 52, 320, 2, RGB332_YELLOW, FILLED);

    os_draw_rectangle(0, 70, 320, 2, RGB332_YELLOW, FILLED);
    os_draw_rectangle(0, 72, 320, 3, RGB332_RED, FILLED);
    os_draw_rectangle(0, 75, 320, 2, RGB332_YELLOW, FILLED);

    os_draw_rectangle(0, 160, 320, 12, RGB332_ORANGE, FILLED);
    os_draw_line(0, 164, 319, 164, RGB332_BLACK);
    os_draw_line(0, 168, 319, 168, RGB332_BLACK);
    os_draw_rectangle(0, 172, 320, 12, RGB332_RED, FILLED);
    os_draw_line(0, 176, 319, 176, RGB332_BLACK);


}

void update_score_kungfu(){
    char score_str[7]; // Buffer for 3 digits + null terminator

    os_draw_rectangle(22, 2, 50, 7, RGB332_SKY_BLUE, FILLED); // Clear previous score
    // Convert the number to string and print it
    int_to_ascii(score_kungfu, score_str);
    os_set_char_xy(22, 2);
    os_set_char_color(RGB332_GREEN);
    os_puts(score_str);
    os_puts("000");
...
void update_time_kungfu(){
    char time_str[7]; // Buffer for 3 digits + null terminator

    os_draw_rectangle(250, 26, 25, 7, RGB332_SKY_BLUE, FILLED); // Clear previous time
    // Convert the number to string and print it
    int_to_ascii_dec(time_remaining_kungfu, time_str);
    os_set_char_xy(250,26);
    os_puts(time_str);
}

void start_kungfu()
{
    time_remaining_kungfu = 2000;
    score_kungfu = 0;

    draw_kungfu_background();
    update_score_kungfu();
    os_play_music(MUSIC_KUNGFU_THEME);

    int nes_x = 160, nes_y = 120;
    int old_draw_x = 160, old_draw_y = 120;
    int bad_x = 0, bad_y = 120;
    int old_bad_x = 0, old_bad_y = 120;
    int bad_active = 0, bad_dying = 0;
    
    unsigned char old_pad = 0;
    int facingRight = 0;
    int lastSpriteDrawn = -1, lastBadSprite = -1;
    long frameCounter = 0;

    // --- PRIME BUFFERS ---
    os_tile_backup_32x44(old_draw_x, nes_y);
    os_tile_backup_32x44b(old_bad_x, bad_y);
    ...
    while(1) {
        ...
        int spriteToShow = 0;
        int offsetX = 0;
        int draw_y = 120; // Default vertical position
        frameCounter++;

        // 1. THOMAS MOVEMENT
        if (pad & NES_LEFT) { nes_x -= 5; thomas_moved = 1; facingRight = 0; }
        else if (pad & NES_RIGHT) { nes_x += 5; thomas_moved = 1; facingRight = 1; }

        // 2. BAD GUY LOGIC
        if (frameCounter > 60 && !bad_active && !bad_dying) bad_active = 1;
        ...
        if (bad_active) {
            if (bad_dying) {
                ...
                bad_sprite = 35;
                if (bad_y > 240) {
                    os_tile_restore_32x44b(old_bad_x, old_bad_y);
                    bad_active = 0;
                    bad_dying = 0;
                    bad_x = 0; 
                    bad_y = 120;
                ...
                if (frameCounter % 2 == 0) bad_x += 5;
                if (bad_x >= (nes_x - 50) && bad_x < nes_x) {
                    bad_sprite = 32 + ((bad_x / 10) % 2);
                } else {
                    bad_sprite = 30 + ((bad_x / 10) % 2);
                }
            }
        }

        // 3. THOMAS ATTACK & OFFSET LOGIC
        int walkFrame = (nes_x / 10) % 2; 
        int isAttacking = 0;

        if ((pad & NES_A) && !(old_pad & NES_A)) os_play_sfx(5); 
        if ((pad & NES_B) && !(old_pad & NES_B)) os_play_sfx(6); 

        if (pad & NES_DOWN) {
            draw_y = 126; // Lower Thomas by 6 pixels when crouching
            if (pad & NES_A) { spriteToShow = 7; offsetX = -4; isAttacking = 1; }
            else if (pad & NES_B) { spriteToShow = 9; offsetX = -12; isAttacking = 1; }
            else spriteToShow = 6;
        }
        else if (pad & NES_A) { spriteToShow = 5; isAttacking = 1; }
        else if (pad & NES_B) { spriteToShow = 4; isAttacking = 1; }
        else if (thomas_moved) spriteToShow = walkFrame;
        else spriteToShow = 0;

        if (facingRight) { spriteToShow += 15; offsetX = -offsetX; }
        
        // Hit Detection (20px Range)
        int distance = (nes_x > bad_x) ? (nes_x - bad_x) : (bad_x - nes_x);
        if (bad_active && !bad_dying && isAttacking && distance < 20) {
            bad_dying = 1;
            score_kungfu += 1;
            update_score_kungfu();
        }
        ...
        // 4. BOUNDARIES
        if (nes_x < 0) nes_x = 0; 
        if (nes_x > 288) nes_x = 288;
        int current_draw_x = nes_x + offsetX;

        // 5. GRAPHICS BURST
        int thomas_changed = (current_draw_x != old_draw_x || draw_y != old_draw_y || spriteToShow != lastSpriteDrawn);
        int bad_changed = (bad_active && (bad_x != old_bad_x || bad_y != old_bad_y || bad_sprite != lastBadSprite));

        if (thomas_changed || bad_changed) {
            if (bad_active || bad_dying) os_tile_restore_32x44b(old_bad_x, old_bad_y); 
            os_tile_restore_32x44(old_draw_x, old_draw_y); // Use old Y to clear correctly
            
            os_tile_backup_32x44(current_draw_x, draw_y);
            if (bad_active) os_tile_backup_32x44b(bad_x, bad_y);
            
            os_draw_sprite32x44(current_draw_x, draw_y, spriteToShow);
            if (bad_active) os_draw_sprite32x44(bad_x, bad_y, bad_sprite);
        }

        // 6. UPDATE STATE
        old_draw_x = current_draw_x;
        old_draw_y = draw_y; // Store current Y for next frame's restore
        old_bad_x = bad_x;
        old_bad_y = bad_y;
        old_pad = pad;
        lastSpriteDrawn = spriteToShow;
        lastBadSprite = bad_sprite;

        if (bad_x > 315 && !bad_dying && bad_active) { 
            os_tile_restore_32x44b(old_bad_x, old_bad_y);
            bad_x = 0; bad_y = 120; old_bad_x = 0; old_bad_y = 120;
            os_tile_backup_32x44b(bad_x, bad_y);
        }

        if (pad & NES_SELECT)
        {
            os_play_music(MUSIC_OFF);
            return;
        }
        if (pad & NES_START)
        {
            if(music_playing_kungfu==0)
            {
                os_play_music(MUSIC_KUNGFU_THEME);
                music_playing_kungfu=1;
                delayms(500);
            }
            else
            {
                os_play_music(MUSIC_OFF);
                music_playing_kungfu=0;
                delayms(500);
            }

        }
        delayms(100); 
        time_remaining_kungfu--;
        update_time_kungfu();
    }
}