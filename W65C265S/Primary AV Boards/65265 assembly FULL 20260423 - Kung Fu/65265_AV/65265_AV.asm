...
; *************** MEMORY MAP **********************************
	...
	; 	$D8:0010 to $DF:FFFF	CS7B				** UNUSED **
	; 	$D8:0000 to $D8:000F	CS7B GAL			VIA Expansion
	; 	$D0:0010 to $D0:FFFF	CS7B				** UNUSED **
	; 	$D0:0000 to $D0:000F	CS7B GAL			VIA Expansion
...
START265:
	...
	; delay to let things settle (e.g., PS/2 keyboard reset, UART) -- or to lest Primary get reset pulled down
	ldx #3000			; works well at 10 MHz
	jsr Delay_ms

	jsr play_tone_startup_start
	jsr print_string_vga_power_on
	...
	; ****** NEW sound ******
	jsr Init_Audio_Engine   ; prepare VIA0 and PSGs
...
IRQHandler_IRQB:
    ...
    phy 
    phd 
    phb                 ; Save caller's Bank
    php                 ; Save caller's Status

    lda #$0000          ; Reset DP to 0
    tcd                 
    
    sep #$20            ; 8-bit for Bank and VIA
    .setting "RegA16", false

    lda #$00
    pha
    plb                 ; DB is now 0. (This matches its own push/pull)

    ; --- Everything below here is safe ---
    lda VIA0_IFR
    bit #%01000000
    beq IRQB_Out
...
IRQB_Out:
    ...
    plp                 ; Restore Status (Mode)
    plb                 ; Restore caller's Bank
