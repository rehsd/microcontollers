...
rtc_get_date:
    ...
    jsr spi_rtc_readbyte
    and #$7F                ; <--- STRIP CENTURY BIT ($84 -> $04)
    sta RTC_MONTH
...
rtc_set_time:
    ...
    lda #$80                ; Write address: seconds
    jsr spi_rtc_writebyte
    lda SYSCALL_PARAMS+5    ; Seconds from shell
    jsr spi_rtc_writebyte
    lda SYSCALL_PARAMS+4    ; Minutes from shell
    jsr spi_rtc_writebyte
    lda SYSCALL_PARAMS+3    ; Hours from shell
    jsr spi_rtc_writebyte
...
rtc_set_date:
    ...
    lda #$84                ; Write address: day
    jsr spi_rtc_writebyte
    lda SYSCALL_PARAMS+2    ; Day from shell
    jsr spi_rtc_writebyte
    lda SYSCALL_PARAMS+1    ; Month from shell
    jsr spi_rtc_writebyte
    lda SYSCALL_PARAMS+0    ; Year from shell
    jsr spi_rtc_writebyte
...
sdcard_init:
    ...
    lda #$00                    ; place error code in A for caller (0 = success)
    ; TO DO - improve logic to prevent endless failure loops above and instead exit with error code after some number of tries
...
sdcard_read_sector:
    ...
    rs_exit:
        pha
        lda #SPI_CS_IDLE   ; set CS high again BEFORE extra clocks
        sta VIA2_PORTA
        lda #$ff           ; extra 8 clocks for the card to finish
        jsr spi_sdcard_writebyte 
