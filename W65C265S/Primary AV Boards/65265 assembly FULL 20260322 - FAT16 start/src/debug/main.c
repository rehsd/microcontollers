...
// static unsigned long final_target_lba;
// static int final_target_idx;
// static unsigned short new_cluster;
...
void print_hex(unsigned char val)
{
    ...
    os_print_char(hex_digits[high]);
    os_print_char(hex_digits[low]);
}
...
// Specialized for 512-byte sector copies
...
void fat16_test()
{
    ...
    os_newline();
    unsigned char result = sdcard_init();
    if (result == 0) {
