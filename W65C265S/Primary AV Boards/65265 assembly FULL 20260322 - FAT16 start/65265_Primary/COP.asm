...
; COP support commands
    ...
    COP_sdcard_read_sector:
        ; Using $1200 for 512-byte shared buffer for SD card read/write operations for now
        ; This fill be filled in by this routine and then read by the caller on return
    ...
    COP_sdcard_write_sector:
        ; Using $1200 for 512-byte shared buffer for SD card read/write operations for now
        ; This fill be written to the SD card by this routine (data should be placed there by the caller before invoking this COP command)
    ...
    COP_get_date_time:
        ...
        jsr rtc_get_date        ; Populates RTC_YEAR, RTC_MONTH, RTC_DAY
        jsr rtc_get_time        ; Populates RTC_HRS, RTC_MIN, RTC_SEC

        ; Store to SYSCALL_PARAMS in requested order: Y, M, D, H, M, S
        lda RTC_YEAR
