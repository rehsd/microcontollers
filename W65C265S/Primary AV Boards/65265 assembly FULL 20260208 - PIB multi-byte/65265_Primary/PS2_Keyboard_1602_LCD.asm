...
.setting "RegXY16", true
	...
	key_pressed:
		...
		beq do_enter_key
			...
			ldx cmd_index
			cpx #$3e              ; check for 62 char limit
			bcs process_out
			sta cmd_buffer,x
			inc cmd_index
		...
		do_enter_key:
			...
			phx
			ldx cmd_index
			lda #0                  ; in 8-bit mode, this is just one byte
			sta cmd_buffer,x        ; terminates the string
			
			; execute & reset index for next time
			jsr execute_command
			stz cmd_index
	...
	Init_Keyboard:
		...
		lda #$0000
		sta cmd_buffer      ; null-terminate the first byte
