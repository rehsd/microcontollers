...
print_hex_serial:
        php             ; save caller status (1 byte)
        sep #$20        ; 8-bit A for logic
        .setting "RegA16", false

        pha             ; [Stack -1] save original byte
        ...
        ; ----- low nibble -----
        pla             ; [Stack Balanced] get original byte back
        pha             ; [Stack -1] save it again so we can clean up at the end
        and #$0f
        jsr nibble_to_ascii
        jsr uart3_tx

        pla             ; [Stack Balanced] final cleanup pull
        plp             ; [Stack Balanced] restore status
        rts

    nibble_to_ascii:
        and #$0f        ; ensure we only have 4 bits
        cmp #$0a
        bcc is_digit
        ; if we are here, carry is SET because A >= 10
        adc #$60        ; $61 - 10 = $57 ('a'-10)
        ; ^ note: i didn't use clc because carry is already set!
        ; $0a + carry(1) + $60 = $6b... wait, let's stick to your clc logic
        ; for clarity to avoid carry-math headaches:
        clc
        adc #$57        ; 'a' - 10
        rts
    is_digit:
