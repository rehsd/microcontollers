...
; ************** TO DO ****************************************
	; Reserve DF00:DFFF as it's reserved by the '265 (and verify any other spaces) - if actually req'd
...
; *********** OS & SHELL SPACE *****************************************
;
; Operating System (primary ROM)
;	* Code runs from ROM at $00:8000 to $00:FFFF
;	* Stack at $00:7FFE (4K)
;	* Bank 0 Variables at $00:0000 to $00:1FFF
;	* Direct page at $00:0000 to $00:00FF (first 256 bytes of Bank 0)
; Shell (C)
...
;	* Bank 0: Variables in $00:2000 to $00:5FFF (16K)
;	* 	Direct page is first 256 bytes of Bank 0 memory
;	* 	Stack $00:6000 to $00:6FFF (4K)
...
.org $8000          ;start of usable ROM
	...
	START265:
	    ...
	    jsr Init_PIB_master		; lets AV out of reset and configures PIB for master mode (primary)

		jsr ILI_Init
		;jsr ILI_Test_Pattern
		;jsr ILI_Animated_Ship		

		; to do: adjust this following delay down
		;ldx #1000				; let AV complete startup - maybe change to a GPIO signal from AV later
		;jsr Delay_ms
		...
		jsr Init_Mouse_Gamepad

		ldx #1000				; let AV complete startup - maybe change to a GPIO signal from AV later
		jsr Delay_ms

		; load the shell
        lda #<STR_LOADING_SHELL
        sta Str_ptr
        lda #>STR_LOADING_SHELL
        sta Str_ptr+1
        jsr pib_puts		; why does this fail?
	...
	Configure_MCU:
		...
		LDA #%11001001	; ENABLE EXT ROM, NMIB, EMULATION MODE, EXT BUS		(BCR5 is ABORTB; it is mutally exclusive with BCR6 NMIB. BCR4 is watchdog. BCR1 and BCR2 are for TG0 and TG1).
		STA BCR
		...
		; **** PIB stuff *****
		lda PDD6
		;ora #%00110001		; diag LED P64, diag LED P65, AV reset
		ora #%00010001		; diag LED, diag LED, AV reset
		sta PDD6
	...
	Init_Misc
		; any miscellaneous initialization that doesn't fit elsewhere
		lda $1234
		sta RNG_SEED
		rts	
	...
	.include "COP.asm"
	.include "SPI.asm"
	.include "PS2_Mouse_SNES_Gamepad.asm"
...
.setting "RegXY16", true
	...
	IRQHandler_IRQB:
        php
        rep #$30                ; 16-bit to save all registers safely
        .setting "RegA16", true
        .setting "RegXY16", true
        pha                     ; Save A (16-bit)
        phx                     ; Save X (16-bit)
        ...
        sep #$20                ; Set A to 8-bit for hardware logic
        .setting "RegA16", false
        lda #$00
        pha
        plb                     ; Force DB=0
		...
		// VIA3_PS2_Mouse:
		// 	lda VIA3_IFR
		// 	and #%00000010          ; Check CA1 for VIA3 (mouse)
		// 	beq VIA0_KBD   ; If not mouse, next

		// 	; --- Mouse Logic Starts Here ---
		// 	// lda #'M'
		// 	// jsr print_char_serial	; temp debug
		// 	lda VIA3_PORTA
		// 	jsr print_hex_serial	; temp debug
		...
		VIA0_KBD:
			...
			beq IRQB_OUT            ; If not keyboard, next
		...
		// VIA3_SNES_Gamepad:
		// 	lda VIA3_IFR
		// 	and #%00010000          ; Check CB1 for VIA3 (SNES gamepad)
		// 	beq IRQB_Out			; If not SNES gamepad, exit (no other devices on IRQB)

		// 	; --- SNES Gamepad Logic Starts Here ---
		// 	lda #'G'
		// 	jsr print_hex_serial	; temp debug
		// 	lda VIA3_PORTB
		// 	jsr print_hex_serial	; temp debug
		...
		IRQB_Out:
			rep #$30                ; 16-bit to restore all registers safely
			.setting "RegA16", true
			.setting "RegXY16", true
...
.org $FFB4	; per 65265 datasheet		; $FFB4 - COP
	.word   cop_handler

.org $FFE0
;					;65C816 Interrupt Vectors -- **!!! DO NOT TRUST THESE FOR 65265 !!!!**
;					;Status bit E = 0 (Native mode, 16 bit mode)
	.word	badVec			; $FFE0 - IRQRVD4(816)
	.word	badVec			; $FFE2 - IRQRVD5(816)
	.word	badVec			; $FFE4 - COP(816)	; NOT for the 65265!!!
...
;					;Status bit E = 1 (Emulation mode, 8 bit mode)
	...
	.word	badVec			; $FFF4 - COP(8 bit Emulation)	!!! shouldn't be needed
