...
; 06 January 2026
; Updated to move PS/2 keyboard from serial shift in on '265 port to external VIA port - using VIA0 Port A from schematic
; Updated to move 1602 LCD output from '265 port to external VIA port' - using VIA0 Port B from schematic


; ************** TO DO ****************************************
;
; -Continue cleanup of 8bit vs 16 bit A, X Registers
; 		-proper php, rep, longa, longi, plp
; 		-optimizing 8-bit vs. 16-bit implementations
...
CODESECT SECTION OFFSET $18000	
	...
	START:
		...
		; ******** Stack pointer *********************************************
			;Mensch monitor uses $01FF for stack pointer
			;ldx #$01FF		; initialize native stack pointer (set S = $01FF)
			;txs
			; Move stack to $7FFF. CPU must be in native mode.
			;php                 ; save P
			;sei
			;phx                 ; save X
			;ldx #$7fff          ; desired stack pointer (16-bit)
			;txs                 ; transfer X -> S (sets stack pointer)
			;plx                 ; restore X
			;plp                 ; restore P (restores I too)
		; ******** /Stack pointer ********************************************


		;lda #$0000		; Make direct page = $0000 (in case it wasn't already)
		;tcd             ; transfer A -> D
		...
		;ldx #2000			; works well at 8 MHz
		
		ldx #3000			; works well at 8 MHz
		jsr Delay_ms
	...
	Configure_MCU:
		...
		; set PD4 pin direction for debug outputs
		; 0 bits are input -- using PD46 for oscilliscope debug output, same for PD47 (pseudo LED)
		lda #%11000000	
		sta PDD4

		;; Configure PS2 pins and interrupt, and IRQB interrupt (used by VIAs)
...
ROM_DATA	SECTION
	...
	; ********** STRING DATA *******************************
	STR_INIT_COMPLETE:		.byte 13, 10, "Initialization complete!", 13, 10, "rehsd W65C265S @ 10 MHz, 1602 LCD, 320x240x1B VGA, PS/2 Keyboard", 13, 10,"Welcome!", 0
